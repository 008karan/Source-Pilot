...
def interpret(question,graph_schema,history=None):
    ...
    try:
        return _structured_response(
            ...
            'breakdown "supplier" is the default: one figure per supplier, for the requirement as a whole. '
            'breakdown "requirement" ONLY when the buyer explicitly asks to go line by line — they say per SKU, '
            'per line, per item, line-wise, for each item, or name specific lines. '
            'A question about which supplier is better, cheaper, faster or stronger overall is "supplier", '
            'even when it concerns the whole requirement and even when several suppliers are named. '
            'If the buyer does not ask for detail, do not give them a table of every line. '
