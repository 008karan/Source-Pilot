...
document.addEventListener('click',async e=>{const b=e.target.closest('[data-action]');if(!b)return;const action=b.dataset.action;const actions=['newRequest','confirmNewRequest','intakeDemo','intakeChat','intakeEdit','intakeConfirm','intakeShare','addIntakeLine','removeIntakeLine','removeIntakeFile','newProposal','compareVisual','askNew','askSuggestion','awardChart','newChat'];if(!actions.includes(action))return;e.stopImmediatePropagation();e.preventDefault();await busy(b,async()=>{
 ...
 if(action==='intakeConfirm'&&b.closest('.offer-actions')){
  const stream=$('.intake-stream');
  if(stream){stream.insertAdjacentHTML('beforeend','<div class="user-message just-arrived"><p>Confirmed — these details are correct.</p></div>'+agentThinking('Locking the requirement\u2026'));seenIntake++;stream.scrollTo({top:1e6,behavior:'smooth'})}
  try{await api('/api/intake/save',{fields:intakeData.fields,items:intakeData.items,confirm:true,expected_version:state.dataset_version})}
